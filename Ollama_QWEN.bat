...
REM Models: Qwen3 abliterated:30b  (chat) + nomic-embed-text-v1.5 (embeddings)
...
ECHO Models: qwen2.5vl:32b  + Nomic Embeddings
...
ECHO   Max Models: 2 (QWEN 3 + Nomic)
...
curl -s http://localhost:11434/api/tags | findstr "nomic" >nul 2>&1
IF ERRORLEVEL 1 (
    ECHO [WARNING] Nomic embedding model may not be installed
    ECHO [INFO] Consider pulling nomic-embed-text if needed
) ELSE (
    ECHO [MODELS] Nomic embedding model appears to be available
)
...
ECHO Models: QWEN3:30b + Nomic Embeddings
ECHO Configuration: Using config.py settings
